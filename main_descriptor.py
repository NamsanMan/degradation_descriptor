...
# descriptor.py가 같은 폴더에 있다고 가정
from descriptor import FrequencyDegradationTarget, DegradationDescriptorNet

# =========================
# 1. 경로 및 하이퍼파라미터 설정 (수정됨)
# =========================
...
# Train 경로 (HR/LR 각각의 train 폴더)
# 주의: 만약 train 폴더 안에 'images'라는 폴더가 한 겹 더 있다면, 끝에 / "images"를 추가해야 함.
# 현재 설정: ROOT/HR/train/[이미지 파일들]
...
# 모델 저장 경로
SAVE_PATH: Path = Path(r"D:\LAB\result_files\des_module2\descriptor_net.pth")
...
class HRLRImageDataset(Dataset):
    def __init__(
        ...
        # 파일 개수 불일치 시 경고 (디버깅 용이성)
        if len(common) != len(hr_files) or len(common) != len(lr_files):
            print(f"[Warning] File count mismatch in {hr_dir.parent.name}/{hr_dir.name}")
            print(f"  HR files: {len(hr_files)}, LR files: {len(lr_files)}, Common: {len(common)}")
...
def main():
    ...
    # 0.15, 0.5는 경험적 수치이므로 LR 이미지 특성에 맞게 필요 시 튜닝 가능
    ...
    print(f"\n[Train] Start training for {NUM_EPOCHS} epochs...")
